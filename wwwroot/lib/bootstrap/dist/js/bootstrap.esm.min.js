...
function bootstrapDelegationHandler(e, t, n) {
	return function i(s) {
		...
		for (let { target: r } = s; r && r !== this; r = r.parentNode) for (const a of o) if (a === r) return hydrateObj(s, { delegateTarget: r }), i.oneOff && EventHandler.off(e, s.type, t, n), n.apply(r, [s]);
	};
}
...
class Config {
	...
	_mergeConfigObj(e, t) {
		...
		return { ...this.constructor.Default, ...("object" == typeof n ? n : {}), ...(isElement(t) ? Manipulator.getDataAttributes(t) : {}), ...("object" == typeof e ? e : {}) };
	}
...
class Dropdown extends BaseComponent {
	...
	show() {
		...
		if (!EventHandler.trigger(this._element, EVENT_SHOW$5, e).defaultPrevented) {
			if ((this._createPopper(), "ontouchstart" in document.documentElement && !this._parent.closest(".navbar-nav"))) for (const e of [].concat(...document.body.children)) EventHandler.on(e, "mouseover", noop);
	...
	_getPopperConfig() {
		...
		return (this._inNavbar || "static" === this._config.display) && (Manipulator.setDataAttribute(this._menu, "popper", "static"), (e.modifiers = [{ name: "applyStyles", enabled: !1 }])), { ...e, ...execute(this._config.popperConfig, [e]) };
	}
...
class Modal extends BaseComponent {
	...
	show(e) {
		this._isShown || this._isTransitioning || EventHandler.trigger(this._element, EVENT_SHOW$4, { relatedTarget: e }).defaultPrevented || ((this._isShown = !0), (this._isTransitioning = !0), this._scrollBar.hide(), document.body.classList.add("modal-open"), this._adjustDialog(), this._backdrop.show(() => this._showElement(e)));
	}
	hide() {
	...
	_showElement(e) {
		...
		t && (t.scrollTop = 0),
			...
			this._queueCallback(
				() => {
					this._config.focus && this._focustrap.activate(), (this._isTransitioning = !1), EventHandler.trigger(this._element, EVENT_SHOWN$4, { relatedTarget: e });
				},
				this._dialog,
				this._isAnimated()
			);
...
class Offcanvas extends BaseComponent {
	...
	show(e) {
		this._isShown ||
			...
			this._queueCallback(
				() => {
					(this._config.scroll && !this._config.backdrop) || this._focustrap.activate(), this._element.classList.add("show"), this._element.classList.remove("showing"), EventHandler.trigger(this._element, EVENT_SHOWN$3, { relatedTarget: e });
				},
				this._element,
				!0
			));
...
class Tooltip extends BaseComponent {
	...
	hide() {
		if (this._isShown() && !EventHandler.trigger(this._element, this.constructor.eventName("hide")).defaultPrevented) {
			if ((this._getTipElement().classList.remove("show"), "ontouchstart" in document.documentElement)) for (const e of [].concat(...document.body.children)) EventHandler.off(e, "mouseover", noop);
	...
	_getTemplateFactory(e) {
		return this._templateFactory ? this._templateFactory.changeContent(e) : (this._templateFactory = new TemplateFactory({ ...this._config, content: e, extraClass: this._resolvePossibleFunction(this._config.customClass) })), this._templateFactory;
	}
	...
	_configAfterMerge(e) {
		return (e.container = !1 === e.container ? document.body : getElement(e.container)), "number" == typeof e.delay && (e.delay = { show: e.delay, hide: e.delay }), "number" == typeof e.title && (e.title = e.title.toString()), "number" == typeof e.content && (e.content = e.content.toString()), e;
	}
	_getDelegateConfig() {
		const e = {};
...
class ScrollSpy extends BaseComponent {
	...
	_process(e) {
		this._activeTarget !== e && (this._clearActiveClass(this._config.target), (this._activeTarget = e), e.classList.add("active"), this._activateParents(e), EventHandler.trigger(this._element, EVENT_ACTIVATE, { relatedTarget: e }));
	}
	_activateParents(e) {
...
class Tab extends BaseComponent {
	...
	_activate(e, t) {
		e &&
			...
			this._queueCallback(
				() => {
					"tab" === e.getAttribute("role") ? (e.removeAttribute("tabindex"), e.setAttribute("aria-selected", !0), this._toggleDropDown(e, !0), EventHandler.trigger(e, EVENT_SHOWN$1, { relatedTarget: t })) : e.classList.add("show");
				},
				e,
	...
	_deactivate(e, t) {
		e &&
			...
			this._queueCallback(
				() => {
					"tab" === e.getAttribute("role") ? (e.setAttribute("aria-selected", !1), e.setAttribute("tabindex", "-1"), this._toggleDropDown(e, !1), EventHandler.trigger(e, EVENT_HIDDEN$1, { relatedTarget: t })) : e.classList.remove("show");
				},
				e,
				e.classList.contains("fade")
			));
